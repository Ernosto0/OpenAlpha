...
@pytest.mark.anyio
async def test_settings_save_and_preserve_secret(monkeypatch: pytest.MonkeyPatch) -> None:
    service = build_test_service()
    monkeypatch.setattr("backend.app.api.routes.settings.settings_service", service)
...
@pytest.mark.anyio
async def test_openai_provider_test_success(monkeypatch: pytest.MonkeyPatch) -> None:
    service = build_test_service()
    monkeypatch.setattr("backend.app.api.routes.settings.settings_service", service)
